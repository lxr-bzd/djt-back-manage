...
function to_select_num(){
	...
	to_page_get_data(Math.ceil(tb_num/3));
...
//創建訪問，獲取數據
function to_page_get_data(pageNum){
	$.ajax({
		url:"new/selectData.do",
	...
	$.ajax({
		...
		success : function(result) {
			...
			$("#up_nav_box .up_nav_btn").on("click", function() {
				...
				to_page_get_data(pageNum);
				$("#backs_update_table").offset({top:0});
...
function save_data(){
	...
	//生成json對象
	var json_one = {d_id:$("#up_table_one_title span").text(),d_data:table_one_data};
	var json_two = {d_id:$("#up_table_two_title span").text(),d_data:table_two_data};
	var json_three = {d_id:$("#up_table_three_title span").text(),d_data:table_three_data};
